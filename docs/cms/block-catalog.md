...
| Carousel | `src/blocks/Carousel/config.ts` | `src/blocks/Carousel/Component.tsx` e `src/blocks/Carousel/CarouselClient.tsx` | Slides por visualizacao `1`, `2`, `3`; navegacao `arrows` ou `arrows-dots`; autoplay `off` ou `on`; temas preset ou `custom` com `background` e `foreground` | Conteudo sequencial com navegacao manual acessivel e autoplay opcional pausavel. |
...
| Cards | `src/blocks/Cards/config.ts` | `src/blocks/Cards/Component.tsx` | `default`, `modalities`; temas preset ou `custom` com `background`, `foreground` e `accent` | Listas editoriais em grade com titulo, resumo, itens, icone, descricao e link opcional; a variant `modalities` cobre modalidades sem duplicar Block. |
