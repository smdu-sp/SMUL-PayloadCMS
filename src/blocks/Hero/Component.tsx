        ...
        <Container size="lg">
          ...
          >
            ...
            >
              ...
              {cta?.label ? (
                <div className="mt-8">
                  <BlockLink appearance="solid" link={cta} />
                </div>
              ) : null}
            </div>
            {split ? (
...
function HeroContentScope({ enabled, children, ...props }: {
  ...
  return enabled ? <MediaColorScope {...props} paint={false}>{children}</MediaColorScope> : <div className={props.className}>{children}</div>;
}
